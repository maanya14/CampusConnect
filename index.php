...
if ( $_SERVER[ 'REQUEST_METHOD' ] == 'POST' ) {
    ...
    if ( $result->num_rows > 0 ) {
        ...
        if ( password_verify( $password, $user[ 'password' ] ) ) {
            ...
            // Redirect to the second page on success
            exit();
        } else {
            // Invalid password
            header( 'Location: signup.php' );
            // Redirect to signup page if password doesn't match
            exit();
        }
    } else {
        // User not found
    ...
    <button type = 'button' name = 'signup' id = 'signupButton' onclick = "window.location.href='signup.php'">Sign Up</button>
    </div>
    </form>
